...
- **Tool-Based Execution**: Equipped with a set of tools to interact with the world, including:
  ...
  - `getWeather`: A mock tool to demonstrate fetching specific data (like weather forecasts).
...
1.  **Goal Input**: The user provides a high-level goal, like "Find the weather in Tokyo tomorrow, convert the temperature to Fahrenheit, and suggest 3 outdoor activities."
2.  **Planning Phase**:
    - The goal is sent to the Gemini API with a detailed system prompt and a strict JSON schema.
    - The AI's task is to act as a "planner," breaking the goal down into a list of tasks, identifying the right tool for each, defining the arguments, and mapping out dependencies (e.g., `task2` depends on the result of `task1`).
3.  **Execution Phase**:
    ...
    - The `toolService` maps each task's tool name to its corresponding implementation function. For example, a `webSearch` task will trigger a call to the Gemini API with Google Search grounding enabled.
...
├── services/
│   ├── geminiService.ts      # Handles all direct communication with the Gemini API (planning, summary, search)
